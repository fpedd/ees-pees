...
class TestFakeEnv(unittest.TestCase):
    ...
    def setUp(self):
        self.env =FakeGym(N=10, num_of_sensors=4, obstacles_each=4,
                 step_range=(1, 1), action_type="discrete",
                 discrete_action_shaping="flatten", obs_len=1)
        self.N = 10
    ...
    def test_Baseenv(self):
            env = self.env
            self.assertEqual(self.env.com.field.shape, (self.N,self.N))
