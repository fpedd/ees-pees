	...
	*/
	...
	while (wb_robot_step(timestep) != -1) {
		...
		*(robot_data.distance) = wb_lidar_get_range_image(lidar);
		...
		// print -20° to 19° distances
		for(int b = 0; b < 40; b++) {
			printf("%i:%.2f - ", b, lidar_data[(res-20+b)%res]);
		}
		printf("\n");
		...
		double v = test_buf.speed * wb_motor_get_max_velocity(motor);
		wb_motor_set_velocity(motor, v);

		// position control for steering
		double p = test_buf.heading; //* (sin(current_time * m_c + m_b) * m_a) + 0.5;
